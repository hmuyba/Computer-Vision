...
def umbralizacion_Demostrativa():
    # 0 para que se lea en escala de grises
    img = cv2.imread('escala.jpg', 0)

    # imagen, umbral o thresh, maxvalue, metodo => umbral e imagen umbralizada
    # 0 negro, maximo blanco
    ...
    for i in range(6):
        # 3 filas, 2 columnas, pos
        plt.subplot(3, 2, i + 1)
        plt.imshow(images[i], 'gray', vmin=0, vmax=255);
        plt.title(labels[i])
        # ejes que encierran a las figuras
...
def umbralizacion_Adaptativa():
    ...
    # imagen, maxvalue, metodo adaptativo, metodo de umbraliz, tam del bloque que tendran los pixeles vecinos, constante
